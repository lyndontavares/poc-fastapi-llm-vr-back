...
# "models/gemini-2.0-flash" #"models/gemini-pro" #"gemini-2.0-flash"
...
UPLOAD_DIR = "uploads"
# Para extração persistente (do /upload_zip)
EXTRACTED_DIR = os.path.join(UPLOAD_DIR, "extracted_csvs")
# Para processamento temporário de ZIPs/CSVs
TEMP_PROCESSING_DIR = os.path.join(UPLOAD_DIR, "temp_processing")
...
async def upload_zip_file(file: UploadFile = File(...)):
    ...
    if not file.filename.lower().endswith(".zip"):
        raise HTTPException(
            status_code=400, detail="Por favor, envie um arquivo no formato .zip")
    ...
    try:
            ...
            for member in zip_ref.namelist():
                if member.lower().endswith('.csv') and not member.startswith('__MACOSX/'):  # Ignora arquivos de metadados do Mac
                    ...
                    target_path = os.path.join(
                        EXTRACTED_DIR, os.path.basename(member))
        ...
        logger.error(
            f"Erro ao processar o upload do ZIP '{file.filename}': {e}", exc_info=True)
        raise HTTPException(
            status_code=500, detail=f"Erro no processamento do arquivo ZIP: {e}")
    finally:
        # Opcional: Remover o arquivo ZIP original após a extração se não for mais necessário
        if os.path.exists(zip_file_path):
            os.remove(zip_file_path)
            logger.info(
                f"Arquivo ZIP '{zip_file_path}' removido após extração.")
...
async def list_extracted_csvs():
    ...
    try:
        csv_files = [f for f in os.listdir(
            EXTRACTED_DIR) if f.lower().endswith('.csv')]
        return {"csv_files": csv_files}
    except Exception as e:
        logger.error(f"Erro ao listar CSVs extraídos: {e}", exc_info=True)
        raise HTTPException(
            status_code=500, detail=f"Erro ao listar arquivos CSV: {e}")
...
async def load_csv_file(csv_filename: str = Form(...)):
    ...
    if not os.path.exists(csv_file_path):
        raise HTTPException(
            status_code=404, detail=f"Arquivo CSV '{csv_filename}' não encontrado.")

    try:
        current_loaded_df = pd.read_csv(csv_file_path)
        current_loaded_csv_name = csv_filename
        logger.info(
            f"Arquivo '{csv_filename}' carregado com sucesso e definido como ativo.")
        return {"message": f"Arquivo '{csv_filename}' carregado com sucesso."}
    except Exception as e:
        logger.error(
            f"Erro ao carregar o arquivo CSV '{csv_filename}': {e}", exc_info=True)
        raise HTTPException(
            status_code=500, detail=f"Erro ao carregar o arquivo CSV: {e}")
...
async def ask_csv_question(request: QuestionRequest):
    ...
    if current_loaded_df is None:
        raise HTTPException(
            status_code=400, detail="Nenhum arquivo CSV está carregado. Por favor, carregue um arquivo primeiro.")

    user_question = request.question
    logger.info(
        f"Pergunta recebida para '{current_loaded_csv_name}': '{user_question}'")
    ...
    **Sua Resposta:**
    ...
    try:
        ...
        logger.info(
            f"Resposta do Gemini para '{current_loaded_csv_name}': '{gemini_answer}'")
        return {"answer": gemini_answer}
    except Exception as e:
        logger.error(f"Erro ao comunicar com o Gemini: {e}", exc_info=True)
        raise HTTPException(
            status_code=500, detail=f"Erro interno ao processar a pergunta: {e}")
...
async def ask_all_extracted_csvs(request: QuestionRequest):
    ...
    """
    all_extracted_csv_files = [f for f in os.listdir(
        EXTRACTED_DIR) if f.lower().endswith('.csv')]

    if not all_extracted_csv_files:
        raise HTTPException(
            status_code=400, detail="Nenhum arquivo CSV foi extraído. Por favor, faça upload de um ZIP primeiro.")

    user_question = request.question
    logger.info(
        f"Pergunta recebida para TODOS os CSVs extraídos (separados): '{user_question}'")
    ...
    for csv_file_name in all_extracted_csv_files:
        ...
        try:
            ...
            logger.warning(
                f"Não foi possível ler o CSV '{csv_file_name}': {e}")
            combined_csv_context += f"\n--- Erro ao ler CSV: '{csv_file_name}' ({e}) ---\n"

    if not combined_csv_context:
        raise HTTPException(
            status_code=500, detail="Nenhum CSV extraído pôde ser lido para fornecer contexto.")
    ...
    **Sua Resposta:**
    ...
    try:
        ...
        gemini_answer = response.text
        logger.info(
            f"Resposta do Gemini para todos os CSVs: '{gemini_answer}'")
        return {"answer": gemini_answer}
    except Exception as e:
        logger.error(
            f"Erro ao comunicar com o Gemini para múltiplos CSVs: {e}", exc_info=True)
        raise HTTPException(
            status_code=500, detail=f"Erro interno ao processar a pergunta para múltiplos CSVs: {e}")
...
async def upload_zip_and_ask_concatenated(file: UploadFile = File(...), question: str = Form(...)):
    ...
    try:
        with open(zip_file_path, "wb") as f:
            f.write(await file.read())
        logger.info(
            f"Arquivo ZIP temporário '{file.filename}' salvo em '{temp_extract_path}'.")
        ...
        with zipfile.ZipFile(zip_file_path, 'r') as zip_ref:
            for member in zip_ref.namelist():
                if member.lower().endswith('.csv') and not member.startswith('__MACOSX/'):
                    source = zip_ref.open(member)
                    target_path = os.path.join(
                        temp_extract_path, os.path.basename(member))
                    with open(target_path, "wb") as target:
                        target.write(source.read())
                    extracted_csv_paths.append(target_path)
        logger.info(
            f"CSVs extraídos para '{temp_extract_path}': {extracted_csv_paths}")

        if not extracted_csv_paths:
            raise HTTPException(
                status_code=400, detail="O arquivo ZIP não contém nenhum arquivo CSV.")
        ...
        for csv_path in extracted_csv_paths:
            try:
                ...
                logger.warning(
                    f"Não foi possível ler o CSV '{os.path.basename(csv_path)}' para concatenação (em memória): {e}")
                continue

        if not list_of_dfs:
            raise HTTPException(
                status_code=500, detail="Nenhum arquivo CSV pôde ser lido para concatenação a partir do ZIP.")

        combined_df = pd.concat(list_of_dfs, ignore_index=True, sort=False)
        logger.info(
            f"CSVs concatenados em memória. DataFrame resultante tem {combined_df.shape[0]} linhas e {combined_df.shape[1]} colunas.")
        ...
        **Sua Resposta:**
        ...
        gemini_answer = response.text
        logger.info(
            f"Resposta do Gemini (concatenado em memória): '{gemini_answer}'")

        return {"answer": gemini_answer}

    except Exception as e:
        logger.error(
            f"Erro no endpoint /upload_zip_and_ask_concatenated: {e}", exc_info=True)
        raise HTTPException(
            status_code=500, detail=f"Erro ao processar a requisição: {e}")
    finally:
        if os.path.exists(temp_extract_path):
            # Remove o diretório temporário e seu conteúdo
            shutil.rmtree(temp_extract_path)
            logger.info(
                f"Diretório temporário '{temp_extract_path}' e seus conteúdos removidos.")
...
async def upload_zip_create_concatenated_csv_and_ask(file: UploadFile = File(...), question: str = Form(...)):
    ...
    concatenated_csv_path = os.path.join(
        temp_session_dir, concatenated_csv_filename)
    ...
    try:
        ...
        with open(zip_file_path, "wb") as f:
            f.write(await file.read())
        logger.info(
            f"Arquivo ZIP temporário '{file.filename}' salvo em '{temp_session_dir}'.")
        ...
        with zipfile.ZipFile(zip_file_path, 'r') as zip_ref:
            for member in zip_ref.namelist():
                if member.lower().endswith('.csv') and not member.startswith('__MACOSX/'):
                    source = zip_ref.open(member)
                    target_path = os.path.join(
                        temp_session_dir, os.path.basename(member))
                    with open(target_path, "wb") as target:
                        target.write(source.read())
                    extracted_csv_paths.append(target_path)
        logger.info(
            f"CSVs extraídos para '{temp_session_dir}': {extracted_csv_paths}")
        ...
        for csv_path in extracted_csv_paths:
            try:
                ...
                logger.warning(
                    f"Não foi possível ler o CSV '{os.path.basename(csv_path)}' para concatenação: {e}")
                continue
        ...
        logger.info(
            f"DataFrame concatenado salvo em '{concatenated_csv_path}'.")
        ...
        **Sua Resposta:**
        ...
        gemini_answer = response.text
        logger.info(
            f"Resposta do Gemini (do arquivo concatenado): '{gemini_answer}'")

        return {"answer": gemini_answer}

    except Exception as e:
        logger.error(
            f"Erro no endpoint /upload_zip_create_concatenated_csv_and_ask: {e}", exc_info=True)
        raise HTTPException(
            status_code=500, detail=f"Erro ao processar a requisição: {e}")
    finally:
        ...
        if os.path.exists(temp_session_dir):
            ...
            logger.info(
                f"Diretório temporário '{temp_session_dir}' e seus conteúdos removidos.")
...
async def upload_zip_and_query_with_pandas_agent(file: UploadFile = File(...), question: str = Form(...)):
    ...
    try:
        ...
        with open(zip_file_path, "wb") as f:
            f.write(await file.read())
        logger.info(
            f"Arquivo ZIP '{file.filename}' salvo temporariamente em '{temp_session_dir}'.")
        ...
        combined_df = pd.concat(list_of_dfs, ignore_index=True, sort=False)
        logger.info(
            f"CSVs concatenados em um único DataFrame para o agente Pandas. Shape: {combined_df.shape}")
        ...
        llm = ChatGoogleGenerativeAI(
            model=GEMINI_PRO_MODEL, temperature=0.0,
        ...
        pandas_agent = create_pandas_dataframe_agent(
            ...
            combined_df,
            verbose=True,
            # TOOL_CALLING, #CHAT_CONVERSATIONAL_REACT_DESCRIPTION, # Ou AgentType.OPENAI_FUNCTIONS
        ...
        logger.error(
            f"Erro no endpoint /upload_zip_and_query_with_pandas_agent: {e}", exc_info=True)
        raise HTTPException(
            status_code=500, detail=f"Erro ao processar a requisição com agente Pandas: {e}")
    finally:
        # 7. Limpar arquivos temporários e o diretório da sessão
        if os.path.exists(temp_session_dir):
            shutil.rmtree(temp_session_dir)
...
async def upload_csv_batch_and_query_with_sql_agent(
    ...
    if not files:
        raise HTTPException(
            status_code=400, detail="Nenhum arquivo CSV foi enviado.")
    ...
    try:
        ...
        for file in files:
            if not file.filename.lower().endswith(".csv"):
                raise HTTPException(
                    status_code=400, detail=f"Arquivo '{file.filename}' não é um CSV. Por favor, envie apenas arquivos CSV.")
            ...
            try:
                ...
                table_name = ''.join(
                    c for c in table_name if c.isalnum() or c == '_').lower()
                if not table_name:  # Handle case where filename results in empty table name
                    # Generate unique name
                    table_name = f"csv_data_{uuid.uuid4().hex[:8]}"
                dfs[table_name] = df
                logger.info(
                    f"CSV '{file.filename}' carregado e pronto para ser uma tabela '{table_name}'.")
            except Exception as e:
                logger.warning(
                    f"Não foi possível ler o CSV '{file.filename}': {e}", exc_info=True)
                raise HTTPException(
                    status_code=500, detail=f"Erro ao ler o CSV '{file.filename}': {e}")

        if not dfs:
            raise HTTPException(
                status_code=400, detail="Nenhum arquivo CSV válido foi processado.")
        ...
        llm = ChatGoogleGenerativeAI(
            model=GEMINI_PRO_MODEL, temperature=0.0, google_api_key=GOOGLE_API_KEY)
        ...
        logger.error(
            f"Erro no endpoint /upload_csv_batch_and_query_with_sql_agent: {e}", exc_info=True)
        raise HTTPException(
            status_code=500, detail=f"Erro ao processar a requisição com agente SQL: {e}")
    finally:
        ...
        if engine:
            engine.dispose()
            logger.info(
                "Conexão com o banco de dados SQLite em memória descartada.")
...
async def ask_all_extracted_csvs_custom_prompt(request: CustomPromptQuestionRequest):
    ...
    if not all_extracted_csv_files:
        raise HTTPException(
            status_code=400, detail="Nenhum arquivo CSV foi extraído. Por favor, faça upload de um ZIP primeiro usando /upload_zip.")

    user_question = request.user_question
    prompt_template = request.prompt_template
    logger.info(
        f"Pergunta recebida (custom prompt) para TODOS os CSVs extraídos: '{user_question}'")
    ...
    if "{csv_context}" not in prompt_template:
        raise HTTPException(
            status_code=400, detail="O template de prompt deve conter o placeholder '{csv_context}'.")
    if "{user_question}" not in prompt_template:
        raise HTTPException(
            status_code=400, detail="O template de prompt deve conter o placeholder '{user_question}'.")

    try:
        final_prompt = prompt_template.format(
            csv_context=csv_context_string, user_question=user_question)
        # Log os primeiros 500 chars
        logger.info(
            f"Prompt final enviado ao Gemini: \n{final_prompt[:500]}...")
    except KeyError as e:
        raise HTTPException(
            status_code=400, detail=f"Erro ao preencher o template do prompt. Placeholder ausente ou incorreto: {e}")
    ...
    try:
        ...
        logger.error(
            f"Erro ao comunicar com o Gemini (custom prompt): {e}", exc_info=True)
        raise HTTPException(
            status_code=500, detail=f"Erro interno ao processar a pergunta com prompt customizado: {e}")
